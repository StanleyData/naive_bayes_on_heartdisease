...
Time permitting begin Part 7 of DQ but make a notebook and a cell for each screen where you apply the code to a dataset. Choose a new dataset and it can have either a binary target variable or continuous target variable
"""
...
with st.sidebar: 
	sideselect = option_menu(
		...
		options = ["Start", "Intro", "Data Cleaning", "Data Exploration", "Explaining Naive Bayes", 'Feature Selection', 'Interactive Classifier', 'Conclusion'],
		menu_icon = 'arrow-down-right-circle-fill',
		icons = ['check-lg', 'book', 'clipboard', 'compass', 'map', 'cart', 'boxes', 'check2-circle'],
		default_index = 0,
		)
...
if sideselect == "Data Cleaning":
    ...
    st.code("num_col_dict = {'BMI':'BMI', 'Age':'Age', 'GenHlth':'General Health', 'MentHlth':'Mental Health', 'PhysHlth':'Physical Health', 'Education':'Education', 'Income':'Income'}")
...
# =============================================================================
        ...
        for col in catcols:
            catcol_dict0[col] = X[col].replace(dict(X0[col].value_counts(normalize = True)))
        for col in catcols:
            catcol_dict1[col] = X[col].replace(dict(X1[col].value_counts(normalize = True)))
        ...
        pred_df0 = pd.concat([catcol_df0, numcol_df0], ignore_index = True, axis = 1)
        ...
        pred_df1 = pd.concat([catcol_df1, numcol_df1], ignore_index = True, axis = 1)
...
if sideselect == 'Feature Selection':
    ...
    st.write("The curse of dimensionality is also a very confusing problem with machine learning models. The term alone was invented by some computer programmers. The amount of data the model needs for it to reach a certain accuracy gets higher and higher when you train the model with more columns.")
    ...
    st.write("This is a dataframe with all of the categorical columns and their chi-squared test information:")
    all_list = []
    for col in cat_col_dict.values():
        badname = [k for k,v in cat_col_dict.items() if v == col][0]
        res_list = chi2_contingency(pd.crosstab(df['HeartDiseaseorAttack'], df[badname]))[0:2]
        all_list.append(res_list)
    new_cols = ["columns", "chi-squared value", "p-value"]
    chi_df = pd.DataFrame(all_list, columns = ["chi-squared value", "p-value"]).assign(columns = df_cat.columns)
    chi_df = chi_df[new_cols]
    st.write(chi_df) # the really small decimals can't be displayed???
    st.write("Let's sort them by the p-value:")
    st.write(chi_df.sort_values(by = ['p-value']))
    st.write("As you can see, the resulting dataframe isn't accurate. Some columns have greater chi-squared values but are put to the back by the sort function.")
    st.write("So, I would sort the dataframe by the chi-squared value.")
    st.write(chi_df.sort_values(by = ['chi-squared value']))
    st.write("That's more like it. Now we can know what features to choose for our classifier.")
    ...
    st.write("4. The **Median** and the **Quartiles**: If the median and quartiles are close together, the data may be too much the same and may not provide enough information for a model to do good predicting.")
